...
type DraftPreset = Omit<SetupFeePreset, "id" | "isActive" | "unitLabel">

const EMPTY_DRAFT: DraftPreset = { name: "", description: null, defaultRate: 0, defaultCost: 0, sortOrder: 0 }

export default function DashboardSettingsViewSetupFeePresets() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<SetupFeePreset | null>(null)
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    fetch("/api/setup-fee-presets?all=1")
      .then((r) => r.json())
      .then(({ data }) => { if (data) setPresets(data) })
  }, [])

  function startEdit(preset: SetupFeePreset) {
    setEditingId(preset.id)
    setEditDraft({ name: preset.name, description: preset.description, defaultRate: preset.defaultRate, defaultCost: preset.defaultCost, sortOrder: preset.sortOrder })
  }

  async function saveEdit(id: number) {
    if (!editDraft.name.trim()) { toast.error("Name is required."); return }
    setSaving(true)
    const res = await fetch(`/api/setup-fee-presets/${id}`, {
      method: "PATCH",
  ...
  async function toggleActive(preset: SetupFeePreset) {
    const res = await fetch(`/api/setup-fee-presets/${preset.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleCreate() {
    ...
    setSaving(true)
    const res = await fetch("/api/setup-fee-presets", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete(preset: SetupFeePreset) {
    const res = await fetch(`/api/setup-fee-presets/${preset.id}`, { method: "DELETE" })
    const json = await res.json()
    if (json.error) { toast.error(json.error); return }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-base font-semibold text-(--color-foreground)">Setup Cost Presets</h3>
          <p className="text-sm text-(--color-muted) mt-0.5">Presets that appear in the &ldquo;Add Setup Cost&rdquo; dropdown on orders.</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => { setCreateDraft(EMPTY_DRAFT); setShowCreateDialog(true) }} className="gap-1">
        ...
        <table className="w-full text-sm min-w-160">
          <thead>
            <tr className="border-b border-(--color-border) bg-(--color-surface)">
              ...
              <th className="text-right px-3 py-2.5 font-medium text-(--color-muted) w-28">Default Rate</th>
            ...
            {presets.map((preset) => {
              ...
              return (
                <tr key={preset.id} className={`border-b border-(--color-border) last:border-0 ${!preset.isActive ? "opacity-50" : ""}`}>
                  ...
                  <td className="px-3 py-2">
                    {isEditing ? (
                      <Input value={editDraft.description ?? ""} onChange={(e) => setEditDraft((d) => ({ ...d, description: e.target.value || null }))}
                  ...
                  <td className="px-3 py-2 text-right">
                    {isEditing ? (
                      <Input type="number" inputMode="decimal" step="0.01" value={editDraft.defaultRate}
                        onChange={(e) => setEditDraft((d) => ({ ...d, defaultRate: Number(e.target.value) }))}
                        className="text-base h-8 w-24 text-right ml-auto" />
                    ) : (
                      <span>${preset.defaultRate.toFixed(2)}</span>
                    )}
                  </td>
                  <td className="px-3 py-2 text-right">
                    {isEditing ? (
                  ...
                  <td className="px-3 py-2 text-center">
                    <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => toggleActive(preset)}
                      title={preset.isActive ? "Deactivate" : "Activate"}>
                      {preset.isActive ? <Eye size={14} className="text-(--color-success)" /> : <EyeOff size={14} className="text-(--color-muted)" />}
                    </Button>
                  </td>
                  <td className="px-3 py-2">
                    <div className="flex items-center justify-end gap-1">
                      {isEditing ? (
                        <>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-(--color-success)" disabled={saving} onClick={() => saveEdit(preset.id)}>
                            <Check size={14} />
                          </Button>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-(--color-muted)" onClick={() => setEditingId(null)}>
                            <X size={14} />
                          </Button>
                        </>
                      ) : (
                        <>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => startEdit(preset)}>
                            <Pencil size={14} />
                          </Button>
                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-(--color-danger)" onClick={() => setDeleteTarget(preset)}>
                            <Trash2 size={14} />
                          </Button>
                        </>
                      )}
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="bg-(--color-background)">
          <DialogHeader><DialogTitle>New Setup Cost Preset</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label>Name *</Label>
              <Input value={createDraft.name} onChange={(e) => setCreateDraft((d) => ({ ...d, name: e.target.value }))}
                className="text-base" placeholder="e.g. Artwork Fee" />
            </div>
            <div className="space-y-1.5">
              <Label>Description</Label>
              <Input value={createDraft.description ?? ""} onChange={(e) => setCreateDraft((d) => ({ ...d, description: e.target.value || null }))}
                className="text-base" placeholder="e.g. Per Color/Design" />
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1.5">
                <Label>Default Rate</Label>
                <Input type="number" inputMode="decimal" step="0.01" value={createDraft.defaultRate}
                  onChange={(e) => setCreateDraft((d) => ({ ...d, defaultRate: Number(e.target.value) }))} className="text-base" />
              </div>
              <div className="space-y-1.5">
                <Label>Default Cost</Label>
                <Input type="number" inputMode="decimal" step="0.01" value={createDraft.defaultCost}
                  onChange={(e) => setCreateDraft((d) => ({ ...d, defaultCost: Number(e.target.value) }))} className="text-base" />
              </div>
              <div className="space-y-1.5">
                <Label>Sort Order</Label>
                <Input type="number" inputMode="numeric" value={createDraft.sortOrder}
                  onChange={(e) => setCreateDraft((d) => ({ ...d, sortOrder: Number(e.target.value) }))} className="text-base" />
              </div>
            </div>
          </div>
          <DialogFooter className="pb-[max(1rem,env(safe-area-inset-bottom))]">
            <Button variant="outline" onClick={() => setShowCreateDialog(false)}>Cancel</Button>
            <Button autoFocus onClick={handleCreate} disabled={saving}>Create Preset</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(v) => { if (!v) setDeleteTarget(null) }}>
        <AlertDialogContent className="bg-(--color-background)">
          ...
          <AlertDialogFooter className="pb-[max(1rem,env(safe-area-inset-bottom))]">
            ...
            <AlertDialogAction autoFocus onClick={() => deleteTarget && handleDelete(deleteTarget)}
